 ...
 *          Update with change to parsNames method to input only first and last 
 *          initials   
 *
 *          1.1 0
 ...
 *
 * @post Provides all data as string containing the name as initials
 *
 * @par Algorithm 
       ...
       ) const
{
    ...
    char initials[STD_STR_LEN];

    parseNames( lastName, firstName, name );

    initials[0] = lastName[0];
    initials[1] = '-';        
    initials[2] = firstName[0];
    initials[3] = NULL_CHAR;

    sprintf( outString, "%s", initials );
}
