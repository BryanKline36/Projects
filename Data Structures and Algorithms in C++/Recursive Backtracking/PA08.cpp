 ...
 * @version 
 *          1.20
 *          Bryan Kline (06 April 2016)
 *          Updated to include recursive backtracking function
 *
 *          1.10
...
bool allClassesUsed( SimpleVector<ClassType> &vector );
       ...
       )
{
    ...
    RoomType thisRoom;
    int index = 0;
    int classVectorSize = classList.getSize();    
    int currentCap;
    bool success = false;

    // condition: there are available classes
    if( !allClassesUsed( classList ) )
    {

        thisRoom = roomList.getAtIndex( roomLevel );
        currentCap = thisRoom.getRoomCapacity();

        while( index < classVectorSize && !allClassesUsed( classList ) )
        {
            thisClass = classList.getAtIndex( index );

            // condition: the current room will fit the class and that class is 
            //            available
            if( currentCap >= thisClass.getSizeRequest() && 
                                                 thisClass.classIsAvailable() )
            {
                thisRoom.setAssociatedIndex( index );
                roomList.setAtIndex( roomLevel, thisRoom );
                thisClass.setClassAvailable( false );

                classList.setAtIndex( index, thisClass );

                // condition: there are still available classes
                if( !allClassesUsed( classList ) )
                {
                    success = fitClassRooms( roomList, classList, 
                                               (roomLevel + 1), showAttempts );
                }
                else
                {                
                     return true;
                ...
                // condition: the previous recursive call didn't sucessfully
                //            fit all the classes
                if( !success )
                {
                    thisClass.setClassAvailable( true );
                    classList.setAtIndex( index, thisClass );
                }                      
            }
...
bool allClassesUsed
       ( 
        SimpleVector<ClassType> &vector
       )
{
    ClassType thisClass;
    int index = 0;

    while( index < ( vector.getSize() ) )
    {
        thisClass = vector.getAtIndex( index );

        // condition: there is a class that's available
        if( thisClass.classIsAvailable() )
        {
            return false;
        }
